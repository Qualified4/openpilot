...
설정은 **186개 항목**으로 구성되며, 아래 메뉴에서 찾을 수 있습니다.

| 대분류 | 항목 수 | 중분류 |
|---|---:|---|
| 주행 제어 | 122 | 시작·오토, 버튼·프리셋, 차량 조향, 속도·감속, 크루즈·차간 |
| 차량·하드웨어 | 18 | 현대·기아, CANFD·HDA, 레이더, 운전자 모니터링, 차량 보조, 기기 하드웨어 |
...
차량·하드웨어에는 18개 항목이 있습니다. 차종·하네스·기기 구성을 정하는 항목은 사용 중인 차량과 장치에 맞춰 설정하세요. CCNC 계기판 표시 세 항목은 아래 안내에 따라 원하는 표시 기능을 선택할 수 있습니다.
...
| [CCNC 계기판 표시](#ccnc-display) | `CcncLaneColor` | CCNC 가속·모드 차선 색상 · 기본 OFF(0) |
| [CCNC 계기판 표시](#ccnc-display) | `CcncModelLanes` | CCNC 모델 차선·변경 애니메이션 · 기본 OFF(0) |
| [CCNC 계기판 표시](#ccnc-display) | `CcncRadarVehicles` | CCNC HDA1 레이더 차량 표시 · 기본 OFF(0) |
...
**차량·하드웨어 → CANFD·HDA**에서 세 항목을 각각 켜거나 끌 수 있습니다. 모두 기본은 꺼짐이며, 변경은 약 1초 이내에 반영됩니다. 계기판 표시를 조절하는 설정입니다.

#### CCNC 가속·모드 차선 색상

차로 강조 색상으로 가감속 상태와 주행 모드를 보여줍니다. 주행 기어에서 작동하며 속도가 높아질수록 강조 구간이 길어집니다. 끄면 기본 색상과 길이를 사용합니다.

주행 모드를 바꾼 뒤 색상이 바뀌기까지 약 1초가 걸릴 수 있습니다. 차선 변경 방향 강조는 아래 차선·변경 애니메이션 항목에서 설정합니다.

#### CCNC 모델 차선·변경 애니메이션

인식한 차선의 곡선과 좌우 위치를 표시하고, 차선 변경 방향을 강조와 전환 애니메이션으로 보여줍니다. 설정속도·제한속도 안내와 조향 보조 상태 아이콘도 함께 바뀌며, 조향 보조가 작동하지 않을 때는 차선을 숨깁니다.

끄면 기본 차선·아이콘 표시를 사용합니다. 트레일러 연결로 차선 변경이 제한되면 해당 제한이 우선합니다. 주변 차량 표시는 다음 항목에서 별도로 설정합니다.

#### CCNC HDA1 레이더 차량 표시

전방 레이더로 감지한 앞차와 좌우 차로 차량을 각각 최대 한 대씩 표시합니다. 커브에 맞춰 표시 위치를 보정하고, 정차 중 주변 차량 표시를 유지합니다. HDA1 CCNC 차량에 적용되며 HDA2에는 적용되지 않습니다. 끄면 기본 차량 표시를 사용합니다.

차선이 잠시 보이지 않아도 계속 감지되는 기존 측면 차량은 최대 2초 또는 자차 이동 30m 중 먼저 도달하는 시점까지 표시를 이어갈 수 있습니다. 차량 감지가 끊기거나 대상이 바뀌면 이 표시 유지가 끝납니다. 이 항목을 켜거나 끌 때는 유지하던 차량 표시를 다시 판단합니다.

후측방 차량 아이콘은 사각지대 감지 알림이며 실제 거리나 정확한 위치를 나타내지 않습니다. 아이콘의 간격으로 차간거리나 차선 변경 가능 여부를 판단하지 마세요.

...
def display(monkeypatch):
  ...
  monkeypatch.setattr(main, "Params", lambda: N(get_bool=lambda key: params[key], get_int=lambda key: 0))
  ...
  source = dict(ALERTS_1=0, ALERTS_2=0, ALERTS_3=0, ALERTS_5=0, SLA_ICON=0,
  ...
  def send(frame=0, flags=HyundaiFlags.CAMERA_SCC.value):
    return main.create_ccnc_messages(N(flags=flags), packer, N(ECAN=0, CAM=2), frame,
                                    N(enabled=True, latActive=True), cs, hud, 0, False, False, 0, False, 0, 0)
